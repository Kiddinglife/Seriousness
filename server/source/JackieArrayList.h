	...
	public:
		...
		void Preallocate(unsigned int countNeeded)
		{
			...
			if (allocation_size < amount2Alloc)
			{
				...
				new_array = JACKIE_INET::OP_NEW_ARRAY<ElementType>(allocation_size, TRACE_FILE_AND_LINE_);
				if (listArray != 0)
				{
					// copy old array over
		...
		void Shrink2MiniSzie(void)
		{
			...
			ElementType * new_array = JACKIE_INET::OP_NEW_ARRAY<ElementType >(mini_size, TRACE_FILE_AND_LINE_);
			// copy old array over
			for (unsigned int counter = 0; counter < list_size; ++counter)
				new_array[counter] = listArray[counter];
		...
		void InsertAtLast(const ElementType &input)
		{
			...
			if (list_size == allocation_size)
			{
				...
				ElementType * new_array = JACKIE_INET::OP_NEW_ARRAY<ElementType >(allocation_size, TRACE_FILE_AND_LINE_);
		...
		void ReplaceAtIndex(const ElementType &input, const ElementType filler, const unsigned int position)
		{
			...
			else
			{
				if (position >= allocation_size)
				{
					...
					ElementType * new_array = JACKIE_INET::OP_NEW_ARRAY<ElementType >
