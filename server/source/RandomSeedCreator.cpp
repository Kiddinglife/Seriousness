...
#define mixBits(u, v)  (hiBit(u)|loBits(v))  // move hi bit of u to hi bit of v
	...
	RandomSeedCreator::RandomSeedCreator()
	{
		left = -1;
	}
	RandomSeedCreator::~RandomSeedCreator()
	...
	void RandomSeedCreator::SeedMT(unsigned int seed)
	{
		printf("%i\n", seed);
		seedMT(seed, state, next, left);
	}
	unsigned int RandomSeedCreator::ReloadMT(void)
	{
		return reloadMT(state, next, left);
	}
	unsigned int RandomSeedCreator::RandomMT(void)
	{
		return randomMT(state, next, left);
	}
	float RandomSeedCreator::FrandomMT(void)
	{
		return frandomMT(state, next, left);
	}
	void RandomSeedCreator::FillBufferMT(void *buffer, unsigned int bytes)
	{
